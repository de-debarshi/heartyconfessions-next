...
const ConfessionService = {
  submitConfession: (confessionData) =>
    axios.post('/api/submit', confessionData).then((res) => res.data),
  fetchConfessions: (pageNumber, filterBy) =>
    axios.get(`/api/list/${filterBy}/${pageNumber}`).then((res) => res.data),
  fetchPopular: () => axios.get(`/api/popular`).then((res) => res.data),
  fetchSingleConfession: (confessionID) =>
    axios.get(`/api/single/${confessionID}`).then((res) => res.data),
